// convert-pdfs.js - Fixed for ImageMagick v7 + Ghostscript
const { execSync } = require('child_process');
const path = require('path');
const fs = require('fs');

async function convertPDFs() {
    console.log('Starting professional PDF conversion...');

    const pdfDir = './sheet-music';
    const outputDir = './sheet-music-images';
    ...
    // Convert each PDF with professional-grade settings for sheet music
    for (const pdfFile of pdfFiles) {
        ...
        // Professional sheet music conversion settings
        const command = [
    ...
    console.log('Images optimized for mobile viewing with native zoom support.');
}
